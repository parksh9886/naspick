...
# Finnhub Removed - Using local consensus_data.json
# FINNHUB_API_KEY = "..." 
...
def load_consensus_data():
    """Load consensus data from local JSON file"""
    try:
        if os.path.exists('consensus_data.json'):
            with open('consensus_data.json', 'r', encoding='utf-8') as f:
                return json.load(f)
    except Exception as e:
        print(f"Error loading consensus_data.json: {e}")
    return {}

CONSENSUS_DATA = load_consensus_data()
print(f"✓ Loaded {len(CONSENSUS_DATA)} consensus records from local file")
...
def analyze_single_stock_context(ticker, hist, final_score_row):
    ...
    try:
        ...
        # [NEW] Wall Street Consensus (Local Data)
        if ticker in CONSENSUS_DATA:
            context['consensus'] = CONSENSUS_DATA[ticker]
            # print(f"      + Consensus attached")
        else:
            context['consensus'] = None
        
        # No rate limiting needed for local lookup
        # time.sleep(3.1) 
...
def main():
    ...
    try:
        ...
        new_history = ranked_df[['Ticker', 'Sector', 'Close', 'Total_Score', 'Rank']].copy()
        new_history['Date'] = latest_date
        
        # Reorder columns
        new_history = new_history[['Date', 'Ticker', 'Sector', 'Close', 'Total_Score', 'Rank']]

        # Load existing history to append
        history_file = 'data/ranking_history.csv'
        if os.path.exists(history_file):
            existing_history = pd.read_csv(history_file)
            # Remove any existing entries for this date to avoid duplicates (idempotency)
            # Convert Date column to datetime to ensure matching works
            existing_history['Date'] = pd.to_datetime(existing_history['Date'])
            new_history['Date'] = pd.to_datetime(new_history['Date'])
            
            existing_history = existing_history[existing_history['Date'] != latest_date]
            
            # Combine
            final_history = pd.concat([existing_history, new_history], ignore_index=True)
            # Sort
            final_history = final_history.sort_values(['Date', 'Rank'])
        else:
            final_history = new_history

        final_history.to_csv(history_file, index=False)
        print(f"✓ Updated ranking history with {len(new_history)} records. Total history: {len(final_history)}")
    except Exception as e:
    ...
    try:
        ...
        # Chart data will be regenerated by generate_chart_data.py in the workflow
        print(f"   ℹ️ Chart data update delegated to generate_chart_data.py")
