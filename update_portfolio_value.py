"""
Daily Portfolio Value Update Script

This script:
1. Loads the current portfolio state from portfolio_state.json
2. Fetches today's prices and rankings from data.json
3. Applies SB1 strategy rules (buy rank<=10, sell rank>50)
4. Calculates new portfolio value
5. Appends a new data point to chart_data.json
6. Saves the updated portfolio state
"""
...
def get_spy_value(start_value, start_date, current_date):
...
def main():
    print("📊 Daily Portfolio Value Update")
    ...
    stock_data = load_json('data.json')
    if not stock_data:
    ...
    chart_data = load_json('chart_data.json') or []
    ...
    if chart_data and chart_data[-1]['date'] == today_str:
        ...
        # Calculate SPY value
        # SPY starts at 100000 on 2020-12-24 (first entry in chart_data)
        spy_value = chart_data[-1].get('spy', 100000) if chart_data else 100000
        
        # Try to get updated SPY value
        try:
    ...
    # Save chart data
    save_json('chart_data.json', chart_data)
    ...
    save_json('portfolio_state.json', portfolio)
